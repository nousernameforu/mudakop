...
REPO="https://api.github.com/repos/nousernameforu/mudakop/releases/latest"
DOWNLOAD_DIR="/tmp/mudakop"
COUNT=3
...
main() {
    ...
    if [ -f "/etc/init.d/mudakop" ]; then
        msg "mudakop is already installed. Upgrading..."
    else
        msg "Installing mudakop..."
    fi

    if command -v curl >/dev/null 2>&1; then
        check_response=$(curl -s "https://api.github.com/repos/nousernameforu/mudakop/releases/latest")
    ...
    if ! ls "$DOWNLOAD_DIR"/*mudakop* >/dev/null 2>&1; then
        msg "No packages were downloaded successfully"
        exit 1
    fi

    for pkg in mudakop luci-app-mudakop; do
        file=""
    ...
    ru=""
    for f in "$DOWNLOAD_DIR"/luci-i18n-mudakop-ru*; do
        if [ -f "$f" ]; then
            ru=$(basename "$f")
            break
        fi
    ...
    if [ -n "$ru" ]; then
        if pkg_is_installed luci-i18n-mudakop-ru; then
                msg "Upgrading Russian translation..."
                pkg_remove luci-i18n-mudakop*
    ...
    find "$DOWNLOAD_DIR" -type f -name '*mudakop*' -exec rm {} \;
}
...
check_system() {
    ...
    if [ "$openwrt_version" = "23" ]; then
        msg "OpenWrt 23.05 не поддерживается начиная с mudakop 0.5.0"
        msg "Для OpenWrt 23.05 используйте mudakop версии 0.4.11 или устанавливайте зависимости и mudakop вручную"
        exit 1
    fi
