...
# Check if string is a valid IPv6 address (including compressed and IPv4-mapped forms)
is_ipv6() {
    local ip="$1"
    local h="[0-9a-fA-F]{1,4}"
    local v4="((25[0-5]|(2[0-4]|1?[0-9])?[0-9])\.){3}(25[0-5]|(2[0-4]|1?[0-9])?[0-9])"
    local regex="^(($h:){7}$h|($h:){1,7}:|($h:){1,6}:$h|($h:){1,5}(:$h){1,2}|($h:){1,4}(:$h){1,3}|($h:){1,3}(:$h){1,4}|($h:){1,2}(:$h){1,5}|$h:(:$h){1,6}|:((:$h){1,7}|:)|::(ffff(:0{1,4})?:)?$v4|($h:){1,4}:$v4)$"

    [[ "$ip" =~ $regex ]]
}

# Check if string is valid IPv6 with CIDR prefix length
is_ipv6_cidr() {
    local value="$1"
...
# Caches the enable_ipv6 setting. Called once from the entrypoint, after config_load
mudakop_IPV6_ENABLED=0

init_ipv6_state() {
    config_get_bool mudakop_IPV6_ENABLED "settings" "enable_ipv6" 0

    # Refuse to half-configure an IPv6 data path on a kernel without IPv6: the nft sets
    # and ip -6 rules would fail one by one and leave routing in an inconsistent state
    if [ "$mudakop_IPV6_ENABLED" -eq 1 ] && [ ! -f /proc/net/if_inet6 ]; then
        log "IPv6 is enabled in the settings but the kernel has no IPv6 support. Continuing with IPv6 disabled" "error"
        mudakop_IPV6_ENABLED=0
    fi
}

# Returns 0 when IPv6 support is turned on in the mudakop settings
ipv6_enabled() {
    [ "$mudakop_IPV6_ENABLED" -eq 1 ]
}
...
# Checks whether the running sing-box was compiled with a given build tag.
# Protocols behind an optional tag are rejected by `sing-box check` with a generic
# "unknown outbound type" error, so testing here produces a message worth reading
sing_box_has_build_tag() {
    local tag="$1"

    sing-box version 2> /dev/null | grep '^Tags:' | grep -q "$tag"
}
...
url_get_host() {
    ...
    case "$url" in
    # Bracketed IPv6 literal, with or without a port: [2001:db8::1]:443
    \[*\]*)
        url="${url#\[}"
        echo "${url%%\]*}"
        return
        ;;
    # Bare IPv6 literal: two or more colons means there is no host:port split
    *:*:*)
        echo "$url"
        return
        ;;
...
url_get_port() {
    ...
    \[*\]:*)
        ...
        return
        ;;
    # Bracketed IPv6 literal without a port, or a bare IPv6 literal
    \[*\] | *:*:*)
        echo ""
        return
        ;;
...
# or IP addresses/subnets, and returns a comma-separated string of valid items.
# IPv6 entries are only accepted when IPv6 support is enabled.
# Arguments:
    ...
    while IFS= read -r line; do
        ...
        subnets)
            if ! is_ipv4_ip_or_ipv4_cidr "$line"; then
                if ! ipv6_enabled || ! is_ipv6_ip_or_ipv6_cidr "$line"; then
