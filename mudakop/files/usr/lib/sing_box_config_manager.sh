...
#     . /usr/lib/mudakop/sing_box_config_manager.sh
#
...
sing_box_cm_patch_dns_route_rule() {
    ...
    echo "$config" | jq \
        ...
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/mudakop"};
        .dns.rules |= map(
...
#######################################
# Add a NaiveProxy outbound to the outbounds section of a sing-box JSON configuration.
# Requires sing-box >= 1.13.0 built with the with_naive_outbound tag.
# TLS is mandatory for this protocol, so the block is always emitted. Only the options
# sing-box documents as meaningful for naive are set: alpn, uTLS fingerprints and Reality
# do not apply here, which is why this does not reuse the generic TLS helper.
# Arguments:
#   config: string (JSON), sing-box configuration to modify
#   tag: string, identifier for the outbound
#   server: string, server address
#   server_port: string or integer, server port
#   username: string, auth username (optional)
#   password: string, auth password (optional)
#   server_name: string, TLS SNI (optional, defaults to the server address)
#   insecure: boolean, "true" to skip certificate verification (optional)
#   quic: boolean, "true" to use HTTP/3 over QUIC instead of HTTP/2 (optional)
#   extra_headers: string (JSON object) of additional HTTP headers (optional)
#   udp_over_tcp: string or integer, UDP over TCP protocol version (optional)
# Outputs:
#   Writes updated JSON configuration to stdout
# Example:
#   CONFIG=$(sing_box_cm_add_naive_outbound "$CONFIG" "naive-out" "example.com" 443 "user" "pass" "sni.example.com" "" "true")
#######################################
sing_box_cm_add_naive_outbound() {
    ...
    local insecure="$8"
    local quic="$9"
    local extra_headers="${10}"
    local udp_over_tcp="${11}"
    ...
    echo "$config" | jq \
        ...
        --arg insecure "$insecure" \
        --arg quic "$quic" \
        --argjson extra_headers "$extra_headers" \
        --arg udp_over_tcp "$udp_over_tcp" \
        '.outbounds += [
            {
                type: "naive",
            ...
            + (if $udp_over_tcp != "" then
                   {udp_over_tcp: {enabled: true, version: ($udp_over_tcp | tonumber)}}
               else {} end)
        ]'
}

#######################################
# Add a raw endpoint JSON object to the endpoints section of a sing-box configuration.
# Arguments:
#   config: string (JSON), sing-box configuration to modify
#   tag: string, identifier for the endpoint
#   raw_endpoint: string, JSON object, the raw endpoint configuration to add
# Outputs:
#   Writes updated JSON configuration to stdout
# Note:
#   Deliberately does not set domain_resolver. On an endpoint that field only resolves the
#   endpoint's own peer address, never the domains of traffic tunneled through it, and
#   pointing it at a resolver detoured through this same endpoint would be circular.
#   Destination domains are resolved by a route rule with action "resolve" instead.
# Example:
#   CONFIG=$(sing_box_cm_add_raw_endpoint "$CONFIG" "wg-ep" '{"type":"wireguard","address":["172.16.0.2/32"],"private_key":"...","peers":[{"address":"127.0.0.1","port":51820,"public_key":"...","allowed_ips":["0.0.0.0/0"]}]}')
#######################################
sing_box_cm_add_raw_endpoint() {
    local config="$1"
    local tag="$2"
    local raw_endpoint="$3"

    echo "$config" | jq \
        --arg tag "$tag" \
        --argjson raw_endpoint "$raw_endpoint" \
        '.endpoints += [(
            $raw_endpoint + {tag: $tag}
        )]'
}
...
sing_box_cm_add_inline_ruleset_rule() {
    ...
    echo "$config" | jq -L /usr/lib/mudakop \
        --arg tag "$tag" \
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/mudakop"};
