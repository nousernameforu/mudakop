...
function createSettingsContent(section) {
  ...
  o = section.option(
    ...
    _("Enable IPv6"),
    _(
      "Route IPv6 traffic in addition to IPv4. Requires working IPv6 on both the WAN and the proxy/VPN server. Leave disabled if unsure: with this off, mudakop answers AAAA queries empty and only IPv4 is routed",
    ),
  );
  o.default = "0";
  o.rmempty = false;
  ...
  o = section.option(
    ...
    "dns_strategy",
    _("DNS Strategy"),
    _("Which address families sing-box resolves for domains handled by mudakop"),
  );
  Object.entries(main.DNS_STRATEGY_OPTIONS).forEach(([key, label]) => {
    o.value(key, _(label));
  });
  o.default = "prefer_ipv4";
  o.rmempty = false;
  o.depends("enable_ipv6", "1");
  ...
  o = section.option(
    ...
    _("Delay in milliseconds before reloading mudakop after interface UP"),
  );
  o.depends("enable_badwan_interface_monitoring", "1");
  o.default = "2000";
  o.rmempty = false;
  ...
  o.cfgvalue = function (section_id) {
    return uci.get("mudakop", section_id, "download_lists_via_proxy_section");
  };
  o.load = function () {
    const sections = this.map?.data?.state?.values?.mudakop ?? {};
  ...
  o = section.option(
    ...
    _("mudakop will not modify your DHCP configuration"),
  );
  o.default = "0";
  o.rmempty = false;
