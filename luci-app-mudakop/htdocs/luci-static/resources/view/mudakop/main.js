...
var IPV4_PATTERN = "(?:(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\\.){3}(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])";
...
var IPV4_REGEX = new RegExp(`^${IPV4_PATTERN}$`);
var IPV6_REGEX = new RegExp(`^(?:${IPV6_PATTERN})$`);
...
function validateIPV6Subnet(value) {
...
// src/validators/validateEndpointJson.ts
...
var NAIVE_SCHEME_REGEX = /^naive\+(https|quic):\/\//;
function validateNaiveUrl(url) {
  try {
    ...
    const hostPortPart = atIndex === -1 ? authAndHost : authAndHost.slice(atIndex + 1);
    if (!hostPortPart) {
      return {
        valid: false,
...
// src/mudakop/methods/custom/getConfigSections.ts
async function getConfigSections() {
  return uci.load("mudakop").then(() => uci.sections("mudakop"));
}

// src/mudakop/methods/shell/callBaseMethod.ts
async function callBaseMethod(method, args = [], command = "/usr/bin/mudakop") {
...
((Mudakop2) => {
  ...
  })(AvailableMethods = Mudakop2.AvailableMethods || (Mudakop2.AvailableMethods = {}));
  ...
  })(AvailableClashAPIMethods = Mudakop2.AvailableClashAPIMethods || (Mudakop2.AvailableClashAPIMethods = {}));
})(Mudakop || (Mudakop = {}));

// src/mudakop/methods/shell/index.ts
var mudakopShellMethods = {
  ...
  getStatus: async () => callBaseMethod(Mudakop.AvailableMethods.GET_STATUS),
  ...
  getClashApiProxies: async () => callBaseMethod(Mudakop.AvailableMethods.CLASH_API, [
  ...
  setClashApiGroupProxy: async (group, proxy) => callBaseMethod(Mudakop.AvailableMethods.CLASH_API, [
  ...
  globalCheck: async () => callBaseMethod(Mudakop.AvailableMethods.GLOBAL_CHECK),
  showSingBoxConfig: async () => callBaseMethod(Mudakop.AvailableMethods.SHOW_SING_BOX_CONFIG),
  checkLogs: async () => callBaseMethod(Mudakop.AvailableMethods.CHECK_LOGS),
  getSystemInfo: async () => callBaseMethod(
    Mudakop.AvailableMethods.GET_SYSTEM_INFO
  )
};

// src/mudakop/methods/custom/getDashboardSections.ts
...
// src/mudakop/methods/custom/getClashApiSecret.ts
...
// src/mudakop/methods/custom/index.ts
var CustomMudakopMethods = {
  getConfigSections,
...
// src/mudakop/methods/fakeip/generateFakeIpTraffic.ts
async function generateFakeIpTraffic() {
  await Promise.allSettled(
    [FAKEIP_CHECK_DOMAIN, IP_CHECK_DOMAIN].map(
      (domain) => withTimeout(
...
// src/mudakop/methods/fakeip/index.ts
var FakeIpTrafficMethods = {
  generateFakeIpTraffic
};

// src/mudakop/services/tab.service.ts
...
// src/mudakop/tabs/diagnostic/helpers/getCheckTitle.ts
function getCheckTitle(name) {
  return `${name} ${_("checks")}`;
}

// src/mudakop/tabs/diagnostic/checks/contstants.ts
...
// src/mudakop/tabs/diagnostic/diagnostic.store.ts
...
// src/mudakop/services/store.service.ts
...
// src/mudakop/services/logger.service.ts
var Logger = class {
  constructor() {
    this.logs = [];
...
// src/mudakop/services/mudakopLogWatcher.service.ts
var MudakopLogWatcher = class _MudakopLogWatcher {
  constructor() {
    this.intervalMs = 5e3;
  ...
  static getInstance() {
    if (!_MudakopLogWatcher.instance) {
      _MudakopLogWatcher.instance = new _MudakopLogWatcher();
    }
    return _MudakopLogWatcher.instance;
  }
...
// src/mudakop/services/core.service.ts
...
// src/mudakop/services/socket.service.ts
...
// src/mudakop/tabs/dashboard/partials/renderSections.ts
...
// src/mudakop/tabs/dashboard/partials/renderWidget.ts
...
// src/mudakop/tabs/dashboard/render.ts
function render() {
  return E(
    "div",
    {
...
// src/mudakop/fetchers/fetchServicesInfo.ts
...
// src/mudakop/tabs/dashboard/initController.ts
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: _("mudakop"),
        value: servicesInfoWidget.data.mudakop ? _("\u2714 Enabled") : _("\u2718 Disabled"),
        attributes: {
          class: servicesInfoWidget.data.mudakop ? "pdk_dashboard-page__widgets-section__item__row--success" : "pdk_dashboard-page__widgets-section__item__row--error"
        }
      },
      {
        key: _("Sing-box"),
...
// src/mudakop/tabs/dashboard/styles.ts
var styles = `
...
// src/mudakop/tabs/dashboard/index.ts
var DashboardTab = {
  render,
  initController,
  styles
...
// src/mudakop/tabs/diagnostic/renderDiagnostic.ts
function render2() {
...
// src/mudakop/tabs/diagnostic/checks/updateCheckStore.ts
...
// src/mudakop/tabs/diagnostic/helpers/getMeta.ts
...
// src/mudakop/tabs/diagnostic/checks/runDnsCheck.ts
...
// src/mudakop/tabs/diagnostic/checks/runSingBoxCheck.ts
...
// src/mudakop/tabs/diagnostic/checks/runNftCheck.ts
...
// src/mudakop/tabs/diagnostic/checks/runFakeIPCheck.ts
    ...
    items: [
      {
        ...
        key: checks.fakeip ? _("Sing-box returns FakeIP addresses") : _("Sing-box does not return FakeIP addresses"),
        value: IP
      },
      ...insertIf(ipv6Checked, [
        {
          state: checks.fakeip6 ? "success" : "error",
          key: checks.fakeip6 ? _("Sing-box returns IPv6 FakeIP addresses") : _("Sing-box does not return IPv6 FakeIP addresses"),
          value: IP6
        }
      ])
    ]
...
// src/mudakop/tabs/diagnostic/partials/renderAvailableActions.ts
...
// src/mudakop/tabs/diagnostic/partials/renderCheckSection.ts
...
// src/mudakop/tabs/diagnostic/partials/renderRunAction.ts
...
// src/mudakop/tabs/diagnostic/partials/renderSystemInfo.ts
...
// src/mudakop/tabs/diagnostic/checks/runSectionsCheck.ts
...
// src/helpers/getVersionCore.ts
function getVersionCore(version) {
  return version.replace(/^v/, "").split("-")[0];
}

// src/mudakop/tabs/diagnostic/helpers/getMudakopVersionRow.ts
function isUnknownVersion(version) {
  return version === "unknown" || version === _("unknown");
}
function getMudakopVersionRow(diagnosticsSystemInfo) {
  const loading = diagnosticsSystemInfo.loading;
  const unknown = isUnknownVersion(diagnosticsSystemInfo.mudakop_version);
  const hasActualVersion = Boolean(diagnosticsSystemInfo.mudakop_latest_version) && !isUnknownVersion(diagnosticsSystemInfo.mudakop_latest_version);
  ...
  if (getVersionCore(version) !== getVersionCore(diagnosticsSystemInfo.mudakop_latest_version)) {
    return {
      key: "mudakop",
      value: version,
      tag: {
...
// src/mudakop/tabs/diagnostic/initController.ts
...
// src/mudakop/tabs/diagnostic/styles.ts
var styles4 = `
...
// src/mudakop/tabs/diagnostic/index.ts
var DiagnosticTab = {
  render: render2,
