  ...
  const data = configSections
    ...
    .map((section) => {
      if (section.connection_type === 'proxy') {
        ...
        if (section.proxy_config_type === 'endpoint') {
          const outbound = proxies.find(
            (proxy) => proxy.code === `${section['.name']}-out`,
          );
          ...
          try {
            parsedType = JSON.parse(section.endpoint_json)?.type;
          } catch {
            parsedType = undefined;
          }

          const proxyDisplayName = parsedType || outbound?.value?.name || '';
