...
// eslint-disable-next-line @typescript-eslint/no-namespace
export namespace Mudakop {
  // Available commands:
  // start                   Start mudakop service
  // stop                    Stop mudakop service
  // reload                  Reload mudakop configuration
  // restart                 Restart mudakop service
  // enable                  Enable mudakop autostart
  // disable                 Disable mudakop autostart
  // main                    Run main mudakop process
  ...
  // check_logs              Show mudakop logs from system journal
  ...
  // show_config             Display current mudakop configuration
  // show_version            Show mudakop version
  // show_sing_box_config    Show sing-box configuration
  // show_sing_box_version   Show sing-box version
  // show_system_info        Show system information
  // get_status              Get mudakop service status
  ...
  export interface ConfigProxyEndpointSection {
    connection_type: 'proxy';
    proxy_config_type: 'endpoint';
  ...
  export interface FakeIPCheckResult {
    fakeip: boolean;
    IP: string;
    // Empty when IPv6 is disabled in the settings
    fakeip6: boolean;
    IP6: string;
  }
  ...
  export interface GetSystemInfo {
    mudakop_version: string;
    mudakop_latest_version: string;
