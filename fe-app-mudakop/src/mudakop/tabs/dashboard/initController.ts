...
import { CustomMudakopMethods, mudakopShellMethods } from '../../methods';
...
async function fetchDashboardSections() {
  ...
  const { data, success } = await CustomMudakopMethods.getDashboardSections();
...
async function handleChooseOutbound(selector: string, tag: string) {
  await mudakopShellMethods.setClashApiGroupProxy(selector, tag);
...
async function handleTestGroupLatency(tag: string) {
  ...
  await mudakopShellMethods.getClashApiGroupLatency(tag);
  await fetchDashboardSections();
...
async function handleTestProxyLatency(tag: string) {
  ...
  await mudakopShellMethods.getClashApiProxyLatency(tag);
  await fetchDashboardSections();
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: _('mudakop'),
        value: servicesInfoWidget.data.mudakop
          ? _('✔ Enabled')
          : _('✘ Disabled'),
        attributes: {
          class: servicesInfoWidget.data.mudakop
