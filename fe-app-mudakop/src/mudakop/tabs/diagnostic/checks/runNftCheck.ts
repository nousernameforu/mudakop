...
import { FakeIpTrafficMethods, mudakopShellMethods } from '../../../methods';
...
export async function runNftCheck() {
  ...
  // Drive traffic through the tproxy rules so the counters below are non-zero
  await FakeIpTrafficMethods.generateFakeIpTraffic();

  const nftablesChecks = await mudakopShellMethods.checkNftRules();
