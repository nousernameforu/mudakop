...
import { MUDAKOP_LUCI_APP_VERSION } from '../../../constants';
...
async function fetchSystemInfo() {
  const systemInfo = await mudakopShellMethods.getSystemInfo();
  ...
  if (systemInfo.success) {
    ...
    store.set({
      diagnosticsSystemInfo: {
        loading: false,
        mudakop_version: _('unknown'),
        mudakop_latest_version: _('unknown'),
...
async function handleRestart() {
  ...
  try {
    await mudakopShellMethods.restart();
  } catch (e) {
...
async function handleStop() {
  ...
  try {
    await mudakopShellMethods.stop();
  } catch (e) {
...
async function handleStart() {
  ...
  try {
    await mudakopShellMethods.start();
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await mudakopShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await mudakopShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await mudakopShellMethods.globalCheck();
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await mudakopShellMethods.checkLogs();
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await mudakopShellMethods.showSingBoxConfig();
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const mudakopEnabled = Boolean(servicesInfoWidget.data.mudakop);
...
function renderDiagnosticSystemInfoWidget() {
  ...
  const renderedSystemInfo = renderSystemInfo({
    items: [
      getMudakopVersionRow(diagnosticsSystemInfo),
      {
        key: 'Luci App',
        value: normalizeCompiledVersion(MUDAKOP_LUCI_APP_VERSION),
      },
      {
        key: 'Sing-box',
