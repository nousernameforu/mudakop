...
import { NAIVE_SCHEME_REGEX, validateNaiveUrl } from './validateNaiveUrl';
...
export function validateProxyUrl(url: string): ValidationResult {
  ...
  if (NAIVE_SCHEME_REGEX.test(trimmedUrl)) {
  ...
  return {
    valid: false,
    message: _(
      'URL must start with vless://, ss://, trojan://, socks4/5://, hysteria2://hy2://, or naive+https://naive+quic://',
    ),
  };
}
