...
const targetRoot = path.resolve(__dirname, '../luci-app-mudakop/po');
...
async function main() {
    ...
    for (const file of files) {
        ...
        if (file === 'mudakop.pot') {
            const potTarget = path.join(targetRoot, 'templates', 'mudakop.pot');
        ...
        const match = file.match(/^mudakop\.([a-zA-Z_]+)\.po$/);
        if (match) {
            const lang = match[1];
            const poTarget = path.join(targetRoot, lang, 'mudakop.po');
